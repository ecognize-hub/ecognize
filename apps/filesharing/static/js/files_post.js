...
document.getElementById("id_button_delete").addEventListener("click", function(e){ var objId = parseInt(document.getElementById('id_input_object_id_del').value); var objType = document.getElementById('id_input_object_type_del').value; deleteFilesystemObj(objType, objId); }, false);
document.getElementById("id_button_share").addEventListener("click", function(e){ var objId = parseInt(document.getElementById('id_input_object_id_share').value); var objType = document.getElementById('id_input_object_type_share').value; updateShareSettings(objType, objId); }, false);
document.getElementById("id_button_rename").addEventListener("click", function(e){ var objId = parseInt(document.getElementById('id_input_object_id_rename').value); var objType = document.getElementById('id_input_object_type_rename').value; renameObject(objType, objId); }, false);
document.getElementById("id_button_createFolder").addEventListener("click", function(e){ var objId = parseInt(document.getElementById('id_input_object_id_newFolder').value); var objType = document.getElementById('id_input_object_type_newFolder').value; createNewFolder(objId); }, false);
document.getElementById("id_button_uploadReplaceFile").addEventListener("click", function(e){ var objId = parseInt(document.getElementById('id_input_object_id_uploadReplace').value); replaceFileContent(objId, false); }, false);
document.getElementById("id_button_uploadNewFile").addEventListener("click", function(e){ var objId = parseInt(document.getElementById('id_input_object_id_uploadNew').value); var objType = document.getElementById('id_input_object_type_uploadNew').value; uploadFile(objId); }, false);
...
function loadOwnObjects(){
    toggleElemsForOwnView(true);
    document.getElementById("id_navtab_receivedfiles").classList.remove("active");
    document.getElementById("id_navtab_yourfiles").classList.add("active");
    loadFolders(0, 0, false);
    loadFiles(0, 0, false);
}

function loadSharedObjects(){
    toggleElemsForOwnView(false);
    document.getElementById("id_navtab_receivedfiles").classList.add("active");
    document.getElementById("id_navtab_yourfiles").classList.remove("active");
    loadFolders(0, 0, true);
    loadFiles(0, 0, true);
}

window.addEventListener("load", getMe, false);
window.addEventListener("load", loadOwnObjects, false);
window.addEventListener("load", loadMyConnections, false);
window.addEventListener("load", loadNationalOrgs, false);
document.getElementById("id_navtab_yourfiles").addEventListener("click", loadOwnObjects, false);
document.getElementById("id_navtab_receivedfiles").addEventListener("click", loadSharedObjects, false);